...
struct EventLoop : public EventLoopBases {
    ...
    PollerEpoll *poller_;
    std::atomic<bool> exit_; // exit_是是否退出时间循环loop()的标志
    int wakeupFds_[2]; // 其他线程唤醒IO线程
    int nextTimeout_;
...
//多线程的事件派发器
struct EventLoopThreadPool : public EventLoopBases {
    EventLoopThreadPool(int sz) : id_(0), loops_(sz), threads_(sz-1) {} 
    virtual EventLoop *allocEventLoop() { // 使用round-robin算法分配EventLoop
        int c = id_++;
        return &loops_[c % loops_.size()];
    }
    void loop();
    EventLoopThreadPool &exit() {
        for (auto &b : loops_) {
            b.exit();
        }
        return *this;
